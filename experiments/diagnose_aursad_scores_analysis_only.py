#!/usr/bin/env python
"""
diagnose_aursad_scores.py

Analysis-only diagnostics for the frozen AURSAD commissioning benchmark.

Purpose
-------
Explain *why* a detector can have reasonable ranking quality (e.g. AUROC) but very
low recall at the strict commissioning operating point.

The script analyzes, where available:
    - calibration healthy scores
    - evaluation healthy scores
    - per-fault anomaly scores
    - conformal thresholds
    - threshold exceedance rates
    - healthy distribution shift
    - anomaly/healthy score overlap
    - per-class recall
    - calibration-tail conservatism

It is deliberately READ-ONLY with respect to detector result folders.

Expected detector result folders
--------------------------------
outputs/aursad/
    targetonly/
    isolation_forest/
...
The script supports two levels of diagnostics:

1) FULL SCORE MODE
   If episode-level score CSVs exist, it produces true score-distribution analyses.
   It searches recursively for CSVs whose filenames contain "score" and then
   standardizes common column aliases automatically.

2) FALLBACK SUMMARY MODE
   If episode-level score files do not exist, it still uses seed-level results and
   per-class recall files to analyze thresholds, FPR/recall behavior, AUROC, and
   class-specific failure patterns. It reports that score-distribution conclusions
   are unavailable rather than fabricating them.

Typical usage
-------------
From repository root:

    python experiments/diagnose_aursad_scores.py

For one detector:

    python experiments/diagnose_aursad_scores.py --detectors targetonly

For a specific commissioning size:

    python experiments/diagnose_aursad_scores.py --n-values 500

Require episode-level score files:

    python experiments/diagnose_aursad_scores.py --require-score-files

Outputs
-------
outputs/aursad/diagnostics/
    diagnostic_seed_summary.csv
    diagnostic_per_class_summary.csv
    score_distribution_summary.csv          # when raw scores are available
    threshold_tail_summary.csv
    healthy_shift_summary.csv                # when raw scores are available
    anomaly_separation_summary.csv           # when raw scores are available
    diagnostic_findings.json

Design principles
-----------------
- No detector is refit.
- No threshold is changed.
- Frozen results remain untouched.
- All analysis is descriptive/diagnostic.
- Missing raw score artifacts are explicitly reported.
- This stage intentionally does not import Matplotlib or create plots.
"""
...
def classify_findings(
    ...
    """
    Produce a machine-readable diagnostic interpretation.

    Important:
    - In FALLBACK mode, healthy_shift/separation/score_summary may be empty.
    - We therefore only make conclusions supported by available artifacts.
    """
    findings = {
        "interpretation_rules": {
            ...
            "calibration_tail_conservatism": (
                "Suggested when conformal thresholds are extreme relative to calibration "
                "scores and recall remains low despite moderate AUROC."
            ),
            "fault_specific_difficulty": (
                "Suggested when some fault classes have materially lower recall than others."
            ),
        },
        "detectors": {},
    }
    ...
    for detector in detectors:
        ...
        if not s.empty:
            ...
            d["largest_N_joint_seed_success_rate"] = float(
                last["joint_seed_success_rate"]
            )

            if "auroc_mean" in last.index and pd.notna(last["auroc_mean"]):
                d["largest_N_auroc_mean"] = float(last["auroc_mean"])

            if float(last["recall_mean"]) < recall_target:
                d["notes"].append(
                    "Recall remains below the commissioning target at the largest analyzed N."
                )
            if float(last["fpr_mean"]) > fpr_budget:
                d["notes"].append(
                    "Mean FPR exceeds the commissioning budget at the largest analyzed N."
                )

        # Only available in full score mode.
        if not healthy_shift.empty:
            ht = healthy_shift[healthy_shift["detector"] == detector]
            if not ht.empty:
                d["healthy_mean_shift_average"] = float(ht["mean_shift"].mean())
                d["healthy_q99_shift_average"] = float(ht["q99_shift"].mean())
                if float(ht["q99_shift"].mean()) > 0:
                    d["notes"].append(
                        "Evaluation-healthy upper-tail scores exceed calibration-healthy "
                        "scores on average; this is consistent with healthy distribution shift."
                    )

        # Only available in full score mode.
        if not separation.empty:
            sep = separation[separation["detector"] == detector]
            if not sep.empty:
                by_class = (
                    sep.groupby("class_label")[
                        "probability_anomaly_score_gt_healthy"
                    ]
                    .mean()
                    .sort_values()
                )
        ...
        # Tail evidence may be available from thresholds even in fallback mode.
        if not threshold_tail.empty:
            tt = threshold_tail[threshold_tail["detector"] == detector]
                ...
                if len(x):
                    ...
                    if rate >= 0.5:
                        d["notes"].append(
                            ...
                            "supporting calibration-tail conservatism as a plausible mechanism."
                        )
