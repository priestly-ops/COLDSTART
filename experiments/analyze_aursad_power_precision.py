...
VERSION = "aursad-power-precision-v1"
...
def ks_power_for_run(
    ...
    """
    Empirical KS power under an injected location shift.

    Scores are standardized using the observed pooled healthy scale. For each
    simulation replicate:
      - sample calibration scores with replacement from observed calibration;
      - sample healthy-eval scores with replacement from observed healthy eval;
      - add delta SD units to the healthy-eval sample;
      - run two-sided KS;
      - record rejection at alpha.

    This preserves empirical distribution shape while testing sensitivity to a
    controlled location-shift alternative.
    """
    ...
    cal_z = (cal - mean) / sd
    healthy_z = (healthy - mean) / sd

    n_cal = len(cal_z)
    n_eval = len(healthy_z)
    results = []

    for delta in shifts:
        rejections = 0
        for _ in range(reps):
            c = cal_z[rng.integers(0, n_cal, size=n_cal)]
            h = healthy_z[rng.integers(0, n_eval, size=n_eval)] + float(delta)
...
def main() -> None:
    ...
    report = {
        ...
        "interpretation": {
            ...
            "healthy_shift_power": (
                "KS power is estimated for an injected location-shift alternative in "
                "pooled-standard-deviation units while resampling the empirical healthy "
                "score distributions. This does not measure power for every possible "
                "distributional shift shape."
            ),
        },
        "limitations": [
